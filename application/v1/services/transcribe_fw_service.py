...
class TranscribeFWService:
    """
    Local CPU transcription using faster-whisper.
    Steps:
      1) Save upload
      2) Extract 16k mono WAV (ffmpeg)
      3) faster-whisper -> segments
      4) Write JSON / SRT / VTT
    """

    ALLOWED_MEDIA = {"mp4","mov","mkv","avi","webm","m4v","mp3","wav","m4a","aac","flac","ogg"}

    @staticmethod
    def allowed_file(filename: str) -> bool:
        return "." in filename and filename.rsplit(".", 1)[1].lower() in TranscribeFWService.ALLOWED_MEDIA
    ...
    def save_upload(file_storage, upload_dir: str = "uploads") -> str:
        ...
        if not TranscribeFWService.allowed_file(filename):
    ...
    def _write_srt(self, segments: List[Dict]):
        with open(self.srt_path, "w", encoding="utf-8") as f:
            for i, seg in enumerate(segments, start=1):
                f.write(f"{i}\n")
                f.write(f"{self._fmt_srt(seg['start'])} --> {self._fmt_srt(seg['end'])}\n")
    ...
    def _write_vtt(self, segments: List[Dict]):
        with open(self.vtt_path, "w", encoding="utf-8") as f:
            f.write("WEBVTT\n\n")
            for seg in segments:
                f.write(f"{self._fmt_vtt(seg['start'])} --> {self._fmt_vtt(seg['end'])}\n")
                f.write(seg["text"].strip() + "\n\n")
    ...
    def process(
        ...
        *,
        ...
        language: Optional[str] = None    # e.g., "en", "ja"
    ) -> TranscribeFWResult:
        # 1) extract wav
        self.extract_audio_16k_mono()

        # 2) transcribe with faster-whisper (CPU)
        ...
        with open(self.json_path, "w", encoding="utf-8") as f:
            json.dump({"segments": segments, "language": language or getattr(info, "language", None)},
                      f, ensure_ascii=False, indent=2)
        self._write_srt(segments)
        self._write_vtt(segments)

        return TranscribeFWResult(
